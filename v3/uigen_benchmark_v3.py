...
def main():
        ...
        if not dummy_master_config_path.exists():
            dummy_master_prompts = { "benchmark_run_name": "Dummy Run", "prompts": [
                { "prompt_id": "example_prompt_001", "prompt_description": "A simple page.",
                  "viewports_to_test": {"desktop": [1280, 720]},
    ...
    html_dir_path = Path(sys.argv[2])
    output_base_dir = Path(sys.argv[3] if len(sys.argv) > 3 else "ui_benchmark_master_runs")
    ...
    run_folder_name_prefix = html_dir_path.name
    if not run_folder_name_prefix or run_folder_name_prefix in ['.', '..']: 
        run_folder_name_prefix = "html_files_run" 

    run_timestamp = time.strftime('%Y%m%d-%H%M%S')
    current_run_output_dir = output_base_dir / f"{run_folder_name_prefix}_{run_timestamp}"
    current_run_output_dir.mkdir(parents=True, exist_ok=True)
    
    for prompt_config_obj in master_config.get("prompts", []):
        prompt_id = prompt_config_obj.get("prompt_id")
        if not prompt_id: print("Skipping prompt with no prompt_id."); continue
        html_file = html_dir_path / f"{prompt_id}.html"
        if not html_file.exists():
            print(f"HTML '{html_file.name}' for prompt '{prompt_id}' not found. Skipping.")
            all_prompts_results.append({"prompt_id": prompt_id, "status": "FILE_NOT_FOUND", "error": f"HTML file {html_file.name} not found.", 
                                        "scores": {"overall": {"percentage_weighted":0}, 
                                                   "technical_quality": {"earned":0, "max":0}, 
                                                   "prompt_adherence": {"earned":0, "max":0}}})
            continue

        analyzer = None
        try:
            print(f"\n\n{'='*10} Analyzing Prompt: {prompt_id} {'='*10}")
            analyzer = UIBenchmarkAnalyzer(html_file, prompt_config_obj, current_run_output_dir, run_timestamp)
            analyzer.run_single_prompt_analysis()
            all_prompts_results.append(analyzer.get_prompt_report_data())
        except WebDriverException as e_wd:
            print(f"WebDriver error for prompt {prompt_id}: {e_wd}")
            all_prompts_results.append({"prompt_id": prompt_id, "status": "WEBDRIVER_ERROR", "error": str(e_wd), 
                                        "scores": {"overall": {"percentage_weighted":0},
                                                   "technical_quality": {"earned":0, "max":0}, 
                                                   "prompt_adherence": {"earned":0, "max":0}}})
        except Exception as e_prompt:
            print(f"Error analyzing prompt {prompt_id}: {e_prompt}")
            import traceback; traceback.print_exc()
            all_prompts_results.append({"prompt_id": prompt_id, "status": "ANALYSIS_ERROR", "error": str(e_prompt), 
                                        "scores": {"overall": {"percentage_weighted":0},
                                                   "technical_quality": {"earned":0, "max":0}, 
                                                   "prompt_adherence": {"earned":0, "max":0}}})
        finally:
    ...
    master_summary = {
        "benchmark_run_name": run_folder_name_prefix, 
        "benchmark_config_file": str(master_config_path.name),
        "html_source_directory": str(html_dir_path), "overall_run_timestamp": run_timestamp,
        "total_prompts_configured": len(master_config.get("prompts", [])),
        "prompts_analyzed_successfully": len([p for p in all_prompts_results if p.get("status") is None or ("ERROR" not in p.get("status","").upper() and "NOT_FOUND" not in p.get("status","").upper())]),
        "aggregate_scores": {
    ...
    for res in all_prompts_results:
        is_success = res.get("status") is None or res.get("status") == "SUCCESS"
        ...
        master_summary["individual_prompt_results"].append({
            "prompt_id": res.get("prompt_id"), "status": res.get("status", "SUCCESS"),
            ...
            "report_directory": res.get("output_directory_for_this_prompt")
        })
        if is_success:
            master_summary["aggregate_scores"]["total_tq_earned"] += tq_earned
            master_summary["aggregate_scores"]["total_tq_max"] += tq_max
            master_summary["aggregate_scores"]["total_adh_earned"] += adh_earned
            master_summary["aggregate_scores"]["total_adh_max"] += adh_max
    ...
    print(f"\n\n{'='*20} MASTER BENCHMARK SUMMARY {'='*20}")
    print(f"Run: {master_summary['benchmark_run_name']}, Total Configured: {master_summary['total_prompts_configured']}, Analyzed OK: {master_summary['prompts_analyzed_successfully']}")
